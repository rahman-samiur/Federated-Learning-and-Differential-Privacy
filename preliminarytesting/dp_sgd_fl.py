...
def get_cifar10_dataloaders(num_clients=100, batch_size=32):
    """
    Loads CIFAR-10 and distributes it among clients.
    """
...
def dp_sgd_client_update(client_id, model, dataloader, epochs, round_num, 
    ...
    # Save client weights
    save_dir = f"results_iid/client_weights/param_nm_{noise_multiplier}_cn_{max_grad_norm}"
...
def run_arm1_experiment(nm, cn):
    """
    Orchestrates the DP-SGD experiment matching flower-test-sv-dp.
    """
    ...
    RESULTS_DIR = "results_iid"
    os.makedirs(RESULTS_DIR, exist_ok=True)
    
    LOG_FILE = os.path.join(RESULTS_DIR, f"evaluation_logsgd_nm{nm_str}_cn{cn_str}.jsonl")
    FINAL_MODEL_FILE = os.path.join(RESULTS_DIR, f"final_modelsgd_nm{nm_str}_cn{cn_str}.pt")
    ...
    # Setup
    print("Setting up DP-SGD experiment (CIFAR-10, LeNet)...")
    global_model = Net()
    client_dataloaders, test_loader = get_cifar10_dataloaders(
